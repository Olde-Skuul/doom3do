;
;Convert a "C" String into an signed long value
;
;long Strtol(char * Src,char ** LastPtr,int Base);
;
;Note : Do NOT use a base larger than 36!
...
strtol	STMDB	SP!,{R4-R5,LR}	;Push my temps onto the stack
...
GotAZ	CMP	Temp,Base		;branch if the digit is too big
	...
	BMI	TooHigh		;Signed overflow?
	BCC	LoopX		;Fetch the next value if not
TooHigh	MOV	Value,#&7FFFFFFF	;Set to maximum signed value!
...
returnERange MOV	Temp,#ERANGE		;errno = ERANGE
	...
	B	FixNeg		;Correct the last negation
...
FixNeg	TST	Flags,#NegateFlag	;Should I negate the answer?
