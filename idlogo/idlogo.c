...
static void Draw3DO(char *FileName)
{
	CCB *six;

	six=LoadCel(FileName,MEMTYPE_CEL);	/* Load in the cel */
...
int main(int32 argc,char *argv[])
{
	if (argc==2) {
		...
		FadeToBlack(&TheScreen,1);	/* Fade out the screen */
		Draw3DO(argv[1]);			/* Draw the 3DO Logo */
		FadeFromBlack(&TheScreen,60);
		WaitVBL(vbitem,2*60);	/* Wait 3 seconds */
